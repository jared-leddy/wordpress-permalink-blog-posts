...
function digitaluniv_blog_generate_rewrite_rules( $wp_rewrite ) {
  $new_rules = array(
...
add_action( 'generate_rewrite_rules', 'digitaluniv_blog_generate_rewrite_rules' );

function digitaluniv_update_post_link( $post_link, $id = 0 ) {
  $post = get_post( $id );
...
add_filter( 'post_link', 'digitaluniv_update_post_link', 1, 3 );
